...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/adcp-profiler-service/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/adcp-profiler-service/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/adcp-profiler-service/Chart.yaml)
